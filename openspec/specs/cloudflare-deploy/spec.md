# cloudflare-deploy Specification

## Purpose
Cloudflare Workers + D1 一体化部署配置。将后端 API 和前端静态资产部署为单一 Worker，使用 D1 作为云端数据库。此 spec 覆盖所有 Cloudflare 平台专属配置，与平台无关的部署配置见 `dev-tooling` DT-007。
## Requirements
