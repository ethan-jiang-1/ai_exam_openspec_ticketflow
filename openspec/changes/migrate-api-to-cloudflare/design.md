...
一个 Worker 同时处理 API 路由和静态资产。通过 `run_worker_first: ["/api/*", "/health"]` 配置，API 请求由 Worker 优先处理，其他请求走静态资产管道（含 SPA fallback）。

**替代方案**: 两个独立 Worker（API Worker + Assets Worker）— 增加部署复杂度，需要配 CORS 和跨 Worker 路由。

**路由分流机制（关键）：**

Wrangler 在 `main` + `assets` 共存时的默认行为是：精确匹配的静态文件直接返回，不匹配的请求交给 Worker。但 Hono 对不匹配的路由返回 404，导致 SPA fallback（`not_found_handling: "single-page-application"`）永远无法触发。

解决方案：`run_worker_first` 配置项控制请求分流顺序：
- 匹配 `/api/*` 和 `/health` 的请求 → Worker 先处理（Hono 路由）
- 其他请求 → 静态资产管道 → 不匹配时触发 SPA fallback → 返回 `index.html`

```
请求 → run_worker_first 匹配？
         │            │
        Yes           No
         │            │
         ▼            ▼
    Worker(Hono)   Assets 查找
    /api/* → JSON    │
    /health → OK     ├─ 找到 → 返回文件
                     └─ 未找到 → SPA fallback → index.html
```
...
`wrangler.jsonc` 添加 D1 binding 和路由分流：
```jsonc
...
- [Worker + Assets 路由分流] → 默认行为下 Worker 拦截所有不匹配 assets 的请求并返回 404，导致 SPA fallback 失效。必须配置 `run_worker_first` 将 API 请求定向到 Worker，其余请求走 assets + SPA 管道。未来新增非 `/api/*` 前缀的 API 路由时需同步更新此配置。
