...
- [ ] 1.2 实现 `apps/server/src/routes/dashboard.ts` GET /api/dashboard handler [DSH-001]
  ...
  - 通过 Drizzle ORM 查询 efficiency（avgResponseMinutes/avgProcessMinutes/reassignCount）
  - 通过 Drizzle ORM 查询 workload（按 completer 统计 assignedCount/inProgressCount/completedThisWeekCount，completedThisWeekCount 从 ticket_history 统计并去重）
  - 通过 Drizzle ORM JOIN ticket_history + tickets 查询 recentActivity（最近 10 条，含 ticketTitle）
  - 角色权限检查（仅 admin + dispatcher）
  - 所有 DB 操作使用 ORM API，禁止原始 SQL。效率指标查询空集合时返回 0 而非 NaN。
...
- [ ] 3.5 修改 `apps/web/src/App.tsx` 添加 `/dashboard` 路由 + 角色守卫 [DSH-003][WF-008]
  - `/dashboard` 在 ProtectedLayout 内
  - DashboardGuard：submitter/completer 重定向到 /workbench/:role
