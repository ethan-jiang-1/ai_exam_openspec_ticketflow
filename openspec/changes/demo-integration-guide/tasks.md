...
- [ ] 1.1 在 `apps/web/src/App.css` 中新增 `.status-badge` 和 `.status-submitted`/`.status-assigned`/`.status-in_progress`/`.status-completed` 四个状态的样式 [WF-009]
...
- [ ] 3.1 创建 `apps/server/src/__tests__/integration.test.ts`，测试完整流转：create → assign → start → complete，验证每步状态变迁 [DG-001]

## 4. 演示说明

- [ ] 4.1 在 `README.md` 的 Demo Roadmap 区域后新增"演示步骤"章节，包含启动服务、三个角色的操作步骤 [DG-002]
