...
- [ ] 1.3 修复 `apps/web/vite.config.ts` proxy 配置：去掉 `rewrite` 规则 [WF-007 Scenario 1]
...
- [ ] 4.1 修改 `apps/web/src/main.tsx`，用 `BrowserRouter` 包裹 `App` [WF-008]
- [ ] 4.2 重写 `apps/web/src/App.tsx`，定义路由：`/` → 角色选择页，`/workbench/*` → Layout 包裹的工作台页面，未匹配路由重定向到 `/` [WF-008 Scenario 1]

## 5. 页面实现（依赖 4.2）

- [ ] 5.1 创建 `apps/web/src/pages/RoleSelect.tsx` + `RoleSelect.css`，三个角色按钮，点击存 localStorage 并跳转，已有角色自动跳转，无效角色留在选择页 [WF-001]
- [ ] 5.2 创建 `apps/web/src/pages/SubmitterWorkbench.tsx`，创建工单表单（title 必填校验）+ 工单列表表格，createdBy 使用当前角色名 "submitter" [WF-003]
- [ ] 5.3 创建 `apps/web/src/pages/DispatcherWorkbench.tsx`，显示 submitted 状态工单 + 指派人输入框 + 指派按钮，无工单时显示提示 [WF-004]
- [ ] 5.4 创建 `apps/web/src/pages/CompleterWorkbench.tsx`，显示 assigned/in_progress 且 assignedTo="completer" 的工单 + 操作按钮（开始处理/完成） [WF-005]

## 6. 样式

- [ ] 6.1 重写 `apps/web/src/App.css`，替换为工作台全局样式（表格、按钮、表单、布局），使用现有 CSS 变量 [WF-003, WF-004, WF-005]
