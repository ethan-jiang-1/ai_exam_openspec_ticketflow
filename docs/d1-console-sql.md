...
```sql
-- 0000: tickets 表（初始版本，后续 0003/0004 补列）
...
-- ========================================
-- T5: assigned, high → completer（含一次改派）
-- ========================================
INSERT OR IGNORE INTO tickets (id, title, description, status, priority, due_date, created_by, assigned_to, created_at, updated_at)
...
在 D1 Console 中执行以下查询确认一切就绪：

```sql
-- 表是否齐全（应返回 tickets, users, ticket_history）
SELECT name FROM sqlite_master WHERE type='table' ORDER BY name;

-- 列是否齐全
PRAGMA table_info(tickets);
PRAGMA table_info(users);
PRAGMA table_info(ticket_history);
...
SELECT action, COUNT(*) AS cnt FROM ticket_history GROUP BY action ORDER BY cnt DESC;
```

预期结果：
- 3 张表（tickets, users, ticket_history）
- tickets 含 priority, due_date 列；users 含 password_hash 列
- tickets: 12+ 行（含已有工单）
- users: 5 行
- ticket_history: 36+ 行（含回填 + 种子数据）
- 状态分布：submitted≥3, assigned≥3, in_progress≥2, completed≥4
- 动作分布：created≥12, assigned≥12, started≥6, completed≥5, reassigned≥1
